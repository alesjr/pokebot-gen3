...
class StatsDatabase:
    ...
    def sync_living_dex_missions(self, missions) -> None:
        """Persist the version-specific mission catalog in this profile database."""
        families = {mission.family for mission in missions}
        for family in families:
            self._execute_write("DELETE FROM living_dex_mission_flags WHERE family = ?", (family,))
            self._execute_write("DELETE FROM living_dex_missions WHERE family = ?", (family,))
        for mission in missions:
            self._execute_write(
                """
                INSERT INTO living_dex_missions (
                    family, mission_id, mission_order, title, category, games,
                    source_document, source_line, definition_markdown,
                    required_level, hunt_until_route_shinies
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    mission.family,
                    ...
                    mission.source_line,
                    mission.definition_markdown,
                    mission.required_level,
                    int(mission.hunt_until_route_shinies),
                ),
            )
            for flag_name in mission.flags:
                self._execute_write(
                    """
                    INSERT INTO living_dex_mission_flags (family, mission_id, flag_name)
                    VALUES (?, ?, ?)
                    """,
                    (mission.family, mission.mission_id, flag_name),
                )
        self._commit()

    def record_living_dex_flags(self, observations: dict[str, tuple[bool, bool]]) -> None:
        observed_at = datetime.now(timezone.utc)
        for flag_name, (supported, value) in observations.items():
            self._execute_write(
                """
                INSERT INTO living_dex_flag_observations (flag_name, supported, value, observed_at)
                VALUES (?, ?, ?, ?)
                ...
                """,
                (flag_name, int(supported), int(value), observed_at),
            )
        self._commit()

    def set_living_dex_mission_status(
        self,
        family: str,
        mission_id: str,
        status: str,
        ...
        self._execute_write(
            """
            INSERT INTO living_dex_mission_progress (
                family, mission_id, status, evidence, observed_at
            ) VALUES (?, ?, ?, ?, ?)
            ON CONFLICT(family, mission_id) DO UPDATE SET
    ...
    def get_living_dex_mission_summary(self) -> dict:
        mission_count = self._cursor.execute("SELECT COUNT(*) FROM living_dex_missions").fetchone()[0]
        observed = self._cursor.execute(
            """
            SELECT COUNT(*), COALESCE(SUM(value), 0), COALESCE(SUM(supported), 0)
            FROM living_dex_flag_observations
            """
        ).fetchone()
        current = self._cursor.execute(
            """
            SELECT progress.mission_id, missions.title, progress.status, progress.evidence
            FROM living_dex_mission_progress AS progress
            JOIN living_dex_missions AS missions
              ON missions.family = progress.family
             AND missions.mission_id = progress.mission_id
            WHERE progress.status IN ('AVAILABLE', 'IN_PROGRESS', 'BLOCKED')
            ORDER BY missions.mission_order
            LIMIT 1
            """
        ).fetchone()
        return {
            "catalogued": int(mission_count),
            "flags_observed": int(observed[0]),
            "flags_true": int(observed[1]),
            "flags_supported": int(observed[2]),
            "route_policy": "complete_shiny_targets_and_required_level",
            "current": (
                None
                if current is None
                else {
    ...
    def _update_schema(self, from_schema_version: int) -> None:
        ...
        if from_schema_version <= 3:
            self._execute_write(dedent("""
                CREATE TABLE living_dex_missions (
                    family TEXT NOT NULL,
                    mission_id TEXT NOT NULL,
                    mission_order INT UNSIGNED NOT NULL,
                    title TEXT NOT NULL,
                    ...
                    required_level INT UNSIGNED DEFAULT NULL,
                    hunt_until_route_shinies INT UNSIGNED NOT NULL DEFAULT 1,
                    PRIMARY KEY (family, mission_id)
                )
                """))
            self._execute_write(dedent("""
                CREATE TABLE living_dex_mission_flags (
                    family TEXT NOT NULL,
                    mission_id TEXT NOT NULL,
                    flag_name TEXT NOT NULL,
                    PRIMARY KEY (family, mission_id, flag_name),
                    FOREIGN KEY (family, mission_id)
                        REFERENCES living_dex_missions (family, mission_id)
                )
                """))
            self._execute_write(dedent("""
                CREATE TABLE living_dex_flag_observations (
                    flag_name TEXT PRIMARY KEY,
                    supported INT UNSIGNED NOT NULL,
                    value INT UNSIGNED NOT NULL,
                    observed_at DATETIME NOT NULL
                )
                """))
            self._execute_write(dedent("""
                CREATE TABLE living_dex_mission_progress (
                    family TEXT NOT NULL,
                    mission_id TEXT NOT NULL,
